...
fn main() {
    ...
    let res = Sha3BitContributionMetamorphicTest::test(&reference_output, &mut SingleBitMutation::new(&HashInput::new(&[97, 98])));
    println!("{}", res);
    println!("{:?}", Sha3BitContributionMetamorphicTest::call(&HashInput::new(&[97, 98])));
}
